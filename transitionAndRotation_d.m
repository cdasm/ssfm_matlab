function re=transitionAndRotation_d(pts, spts)
...
n=mylength(pts);
f=@(x)sqrt(sum(cross(pts-repmat([x(4),x(5),x(6)],[n,1]),(rotateM(x(1),x(2),x(3))*spts')',2).^2,2));

re=lsqnonlin(f,[0,0,0,0,0,0],[],[],getoptimzeoption);
